...
Tabcom1<- Kriege.logdens %>% filter(Community==4) 
...
ggplot(toto)+
  ...
  theme_minimal() +
  coord_fixed()+
  scale_fill_manual(values=c("cyan3","red", "chartreuse4", "darkgoldenrod1", "burlywood4", "aquamarine2", "chocolate2"))
...
gridded(toto1) <- TRUE
    # coerce to raster
rastercom4<- raster(toto1)
rastercom4
plot(rastercom4, main="", xlab="Longitude", ylab="Latitude")

load("data/satellite/chl/rasterChlnew.Rdata")

dis<- disaggregate(rastercom4, fact=(res(rastercom4)/res(rasterchlnew)))
mCom4<- mask(dis, res)
plot(mCom4)

save(mCom4, file="data/ICES/Com4_raster.Rdata")



# Polygons

pol<- rasterToPolygons(mCom4, dissolve=TRUE)
plot(pol, col=pol@data$Clust)

writeOGR(pol, dsn="data/ICES", layer="Com4", driver="ESRI Shapefile")
