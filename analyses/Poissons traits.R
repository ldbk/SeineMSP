...
maturity<- maturity %>% select(Species, tm, Locality)
maturity<- maturity %>% filter(!is.na(Species))
maturity<- maturity %>% filter(!is.na(tm))
maturity<- maturity %>% filter(grepl("North Sea", Locality)|grepl("Channel", Locality)|grepl("Eastern Atlantic", Locality)|grepl("Eastern North Atlantic", Locality)|grepl("NE Atlantic", Locality)|grepl("Northeast Atlantic", Locality))
...
distrib<- distrib %>% select(Species, FAO, NorthernLatitude, SouthernLatitude, WesternLongitude, EasternLongitude)
distrib<- distrib %>% filter(!is.na(Species))


J1<- distrib %>% left_join(species, by = "Species")
J2<- J1 %>% left_join(reprod, by = "Species")
J3<- J2 %>% left_join(Eco, by = "Species")
J4<- J3 %>% left_join(spawning, by = "Species")
J5<- J4 %>% left_join(maturity, by ="Species")

names(J5)[2]<- "Distrib"
names(J5)[7]<- "LifeSpan"
names(J5)[9]<- "DepthShallow"
names(J5)[10]<- "DepthDeep"
names(J5)[11]<- "Migratory"
names(J5)[12]<- "FertilizationType"
names(J5)[13]<- "SpawningFreq"
names(J5)[17]<- "MaturityAge"
names(J5)[18]<- "MaturityLocality"
