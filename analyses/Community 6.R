...
Tabcom1<- Kriege.logdens %>% filter(Community==6) 
...
# According to the majority rule, the best number of clusters is  2 (or 3)
...
gridded(toto1) <- TRUE
    # coerce to raster
rastercom6<- raster(toto1)
rastercom6
plot(rastercom6, main="", xlab="Longitude", ylab="Latitude")

load("data/satellite/chl/rasterChlnew.Rdata")

dis<- disaggregate(rastercom6, fact=(res(rastercom6)/res(rasterchlnew)))
mCom6<- mask(dis, res)
plot(mCom6)

save(mCom6, file="data/ICES/Com6_raster.Rdata")



# Polygons

pol<- rasterToPolygons(mCom6, dissolve=TRUE)
plot(pol, col=pol@data$Clust)

writeOGR(pol, dsn="data/ICES", layer="Com6", driver="ESRI Shapefile")
