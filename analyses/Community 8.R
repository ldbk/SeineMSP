...
Tabcom1<- Kriege.logdens %>% filter(Community==8) 
...
# According to the majority rule, the best number of clusters is  4 (or 6)
...
gridded(toto1) <- TRUE
    # coerce to raster
rastercom8<- raster(toto1)
rastercom8
plot(rastercom8, main="", xlab="Longitude", ylab="Latitude")

load("data/satellite/chl/rasterChlnew.Rdata")

dis<- disaggregate(rastercom8, fact=(res(rastercom8)/res(rasterchlnew)))
mCom8<- mask(dis, res)
plot(mCom8)

save(mCom8, file="data/ICES/Com8_raster.Rdata")



# Polygons

pol<- rasterToPolygons(mCom8, dissolve=TRUE)
plot(pol, col=pol@data$Clust)

writeOGR(pol, dsn="data/ICES", layer="Com8", driver="ESRI Shapefile")
