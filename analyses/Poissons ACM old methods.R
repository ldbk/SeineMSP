...
# Nettoyage tableau
fishtrait<- fishtrait %>% select(-c(Distrib, DepthDeep, DepthShallow))
fishtrait<- fishtrait[rowSums(is.na(fishtrait[,]))<5,]
...
#}
#hist(fishtrait$LifeSpan)
{
fishtrait$LifeSpan<- cut(fishtrait$LifeSpan, c(0, 20, 40, 100), right = FALSE)
fishtrait$FoodTroph<- cut(fishtrait$FoodTroph, c(0, 3, 4, 5), right = FALSE)
fishtrait$MaturityAge<- cut(fishtrait$MaturityAge, c(0, 2, 6, 20), right = FALSE)
}
