...
Tabcom1<- Kriege.logdens %>% filter(Community==3) 
...
gridded(toto1) <- TRUE
# coerce to raster
rastercom3<- raster(toto1)
rastercom3
plot(rastercom3, main="", xlab="Longitude", ylab="Latitude")

load("data/satellite/chl/rasterChlnew.Rdata")

dis<- disaggregate(rastercom3, fact=(res(rastercom3)/res(rasterchlnew)))
mCom3<- mask(dis, res)
plot(mCom3)

save(mCom3, file="data/ICES/Com3_raster.Rdata")



# Polygons

pol<- rasterToPolygons(mCom3, dissolve=TRUE)
plot(pol, col=pol@data$Clust)

writeOGR(pol, dsn="data/ICES", layer="Com3", driver="ESRI Shapefile")
