...
# According to the majority rule, the best number of clusters is  4 (or 10)
...
gridded(toto1) <- TRUE
    # coerce to raster
rastercom5<- raster(toto1)
rastercom5
plot(rastercom5, main="", xlab="Longitude", ylab="Latitude")

load("data/satellite/chl/rasterChlnew.Rdata")

dis<- disaggregate(rastercom5, fact=(res(rastercom5)/res(rasterchlnew)))
mCom5<- mask(dis, res)
plot(mCom5)

save(mCom5, file="data/ICES/Com5_raster.Rdata")



# Polygons

pol<- rasterToPolygons(mCom5, dissolve=TRUE)
plot(pol, col=pol@data$Clust)

writeOGR(pol, dsn="data/ICES", layer="Com5", driver="ESRI Shapefile")
