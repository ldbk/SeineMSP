...
Tabcom1<- Kriege.logdens %>% filter(Community==2) 
...
gridded(toto1) <- TRUE
# coerce to raster
rastercom2<- raster(toto1)
rastercom2
plot(rastercom2, main="", xlab="Longitude", ylab="Latitude")

load("data/satellite/chl/rasterChlnew.Rdata")

dis<- disaggregate(rastercom2, fact=(res(rastercom2)/res(rasterchlnew)))
mCom2<- mask(dis, res)
plot(mCom2)

save(mCom2, file="data/ICES/Com2_raster.Rdata")



# Polygons

pol<- rasterToPolygons(mCom2, dissolve=TRUE)
plot(pol, col=pol@data$Clust)

writeOGR(pol, dsn="data/ICES", layer="Com2", driver="ESRI Shapefile")
