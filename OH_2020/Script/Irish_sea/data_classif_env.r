...
ts.data <- Env %>% dplyr::group_by(Year,Param,Clusters) %>% dplyr::summarise(Values=mean(Values,na.rm=T))
ts <- ggplot(data=ts.data[ts.data$Param=="PPR",])+
  geom_line(aes(x=Year,y=Values,col=Clusters, group=Clusters),lwd=2)+
