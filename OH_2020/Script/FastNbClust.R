...
{  
    ...
    if (distanceM == 1) 
    {
    		md <- parallelDist::parDist(as.matrix(jeu), method="euclidean")	# "dist" function computes and returns the distance matrix computed by using the specified distance measure to compute the distances between the rows of a data matrix.
	  }
    if (distanceM == 2) 
      {
    		md <- parallelDist::parDist(as.matrix(jeu), method="maximum")	
	    }
    if (distanceM == 3) 
      {
    		md <- parallelDist::parDist(as.matrix(jeu), method="manhattan")	
	    }
    if (distanceM == 4) 
      {
    		md <- parallelDist::parDist(as.matrix(jeu), method="canberra")	
	    }
    if (distanceM == 5) 
      {
    		md <- parallelDist::parDist(as.matrix(jeu), method="binary")	
	    }
    if (distanceM == 6) 
     {
    		md <- parallelDist::parDist(as.matrix(jeu), method="minkowski")	
	   }
...
{   # Dis : Total separation between clusters
    ...
    k <- max(cl)
    centers.matrix <- centers(cl,x)
    Distance.centers<-parallelDist::parDist(as.matrix(centers.matrix))
...
Index.Hubert<-function(x, cl)
{
      ...
      meanP<-mean(P)
      variance.matrix <- numeric(0)
      md <- parallelDist::parDist(as.matrix(x), method="euclidean")
      for(j in 1:n) 
      ...
      Q<- as.matrix(parallelDist::parDist(as.matrix(y), method="euclidean"))
      meanQ<-mean(Q)
      for(j in 1:n) 
...
Indice.DB <- function (x, cl, d = NULL, centrotypes = "centroids", p = 2, q = 2) 
{
    ...
    M <- as.matrix(parallelDist::parDist(as.matrix(centers), p = p))
    R <- array(Inf, c(k, k))
    r = rep(0, k)
    for (i in 1:k) {
        for (j in 1:k) {
        ...
        for (bb in (1:B)) {
            ...
            if (bb == 1) {
                ...
                { set.seed(1)
                  ...
                  pp2 <- cutree(fastcluster::hclust(parallelDist::parDist(as.matrix(Xnew)), method = method), 
                    ClassNr)
